...
function clearInputs(clearButton) {
	...
	if (input.classList.contains('form__input_type_login')) {
		document.querySelectorAll('.form__input_type_login').forEach((element) => {
			...
			loginSpan.classList.add('form__span_inactive');
		});
		...
		document.querySelectorAll('.form__input_type_password').forEach((element) => {
			element.value = '';
			element.classList.remove('form__input_type_error');
			passwordSpan.classList.add('form__span_inactive');
		});
	}
}
...
function passwordCheck() {
  const input = document.querySelectorAll('.form__input_type_password')
	if (pwShow == 0) {
		pwShow = 1;
		show(input);
	} else {
...
function transitionInError() {
	...
	loginSpan.classList.remove('form__span_inactive');
	passwordSpan.classList.remove('form__span_inactive');
